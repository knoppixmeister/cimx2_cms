<?php
	...
	if(!defined('IN_PRODUCTION')) {
		if(ENVIRONMENT == 'development') define('IN_PRODUCTION', FALSE);
		else define('IN_PRODUCTION', TRUE);
	}
	...
	class MX2_Exceptions extends CI_Exceptions {
		/**
		...
		function show_error($heading, $message, $template = 'error_general', $status_code = 500) {
			...
			if(IN_PRODUCTION) {
				return $this->mx_show_error($heading, $message, $template, $status_code);
						//$CI->load->view('errors/'.$template.EXT);
						//CI::$APP->load->view('errors/'.$template.EXT, null, TRUE);
			}

			if(!headers_sent()) set_status_header($status_code);
			...
			foreach($trace as $call) {
				if(	isset($call['file']) AND strpos($call['file'], APPPATH) === 0 AND 
					!self::is_extension($call['file'])) 
				{
					$file = $call['file'];
					$line = $call['line'];
		...
		private static function is_extension($file) {
			foreach(array('libraries/', 'core/') as $folder) {
				if(strpos($file, APPPATH . $folder . config_item('subclass_prefix')) === 0) return TRUE;
			}

			return FALSE;
		}
	
		function mx_show_error($heading, $message, $template = 'error_general', $status_code = 500) {
			//Debug::dump(CURRENT_THEME, $heading, $message, $template, $status_code);
			...
			if(defined('CURRENT_THEME')) $theme = CURRENT_THEME;
			else {
				CI::$APP->load->helper('settings/settings');

				$theme = system_get_setting('frontend_theme', 'default');
			}

			if(file_exists(EXTPATH.'themes/'.$theme."/views/errors/".$template.'.php')) {
				include EXTPATH.'themes/'.$theme."/views/errors/".$template.'.php';
			}
			elseif(file_exists(APPPATH.'themes/'.$theme."/views/errors/".$template.'.php')) {
				include APPPATH.'themes/'.$theme."/views/errors/".$template.'.php';
			}
			else include(APPPATH.'errors/'.$template.'.php');

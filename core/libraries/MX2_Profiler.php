<?php
	...
	class MX2_Profiler {
		...
		protected $_query_toggle_count = 250;
		...
		function __construct($config = array()) {
			...
			foreach($this->_available_sections as $section) {
				if(!isset($config[$section])) $this->_compile_{$section} = TRUE;
			}
		...
		protected function _compile_queries() {
			...
			if(count($dbs) == 0) {
				...
				$output .= 	"<table style='border:none; width:100%;'>";
			...
			foreach($dbs as $db) {
				...
				if(count($db->queries) == 0) {
					...
					foreach($db->queries as $key => $val) {
						...
						$output .= "<tr>
										<td style='padding:5px; vertical-align: top;width:1%;color:#900;font-weight:normal;background-color:#ddd;'>".$time."&nbsp;&nbsp;</td>
										<td style='padding:5px; color:#000;font-weight:normal;background-color:#ddd;'>".$val."</td>
									</tr>";
				...
				$output .= "<tr>
								<td style='padding:5px; vertical-align: top;width:1%;color:#900;font-weight:normal;background-color:#ddd;'>".number_format($time_sum, 4)."</td>
							</tr>"; 
		...
		protected function _compile_get() {
			...
			foreach($_GET as $key => $val) {
				...
				//if(is_array($val)) $output .= "<pre>" . htmlspecialchars(stripslashes(print_r($val, true))) . "</pre>";
				//else $output .= htmlspecialchars(stripslashes($val));
		...
		protected function _compile_post() {
			...
			if(count($_POST) == 0) {
				$output .= "<div style='color:#009900;font-weight:normal;padding:4px 0 4px 0'>".lang('profiler_no_post')."</div>";
			}
			else {
				$output .= "\n\n<table style='width:100%'>\n";
		...
		private function _compile_session_data() {
			if(!isset($this->CI->session)) return;
		...
		function run() {
			...
			if($fields_displayed == 0) {
				$output .= '<p style="border:1px solid #5a0099;padding:10px;margin:20px 0;background-color:#eee">'.lang('profiler_no_profiles').'</p>';
			}

			$output .= '</div>';

			//$output .= '<pre>'.print_r(debug_backtrace(0), TRUE)."</pre>";
			
			//return $this->CI->load->view('profiler/profiler', array(), TRUE);

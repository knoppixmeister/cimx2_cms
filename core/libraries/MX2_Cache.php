<?php
	...
	class MX2_Cache {
		...
		function __construct() {
			...
			$this->load->config('mx_cache');

			$this->_path 			= $this->config->item('cache_dir');
			$this->_default_expires = $this->config->item('cache_default_expires');

			if(!is_dir($this->_path)) show_error("Cache Dir not found: $this->_path");	
		}
		...
		function library($library, $method, $arguments = array(), $expires = NULL) {
			if(!class_exists(ucfirst($library))) $this->load->library($library);

			return $this->_call($library, $method, $arguments, $expires);
		}

		function model($model, $method, $arguments = array(), $expires = NULL) {
			if(!class_exists(ucfirst($model))) $this->load->model($model);

			return $this->_call($model, $method, $arguments, $expires);
		}

		function _call($property, $method, $arguments = array(), $expires = NULL) {
			...
			if($expires >= 0) $cached_response = $this->get($cache_file);
			else {
			...
			// Not FALSE? Return it
			if($cached_response !== FALSE && $cached_response !== NULL) return $cached_response;
			else {
				// Call the model or library with the method provided and the same arguments
				$new_response = call_user_func_array(array($this->$property, $method), $arguments);
		...
		function set_dependencies($dependencies) {
			if (is_array($dependencies)) $this->_dependencies = $dependencies;
			else $this->_dependencies = array($dependencies);
		...
		function add_dependencies($dependencies) {
			...
			else $this->_dependencies[] = $dependencies;
		...
		* Retrieve Cache File
		...
		function write($contents = NULL, $filename = NULL, $expires = NULL, $dependencies = array()) {
			...
			if(flock($fp, LOCK_EX)) {
				...
				log_message('error', "MX_Cache was unable to secure a file lock for file at: ".$cache_path);
			...
			// Log success
			log_message('debug', "MX_Cache file written: ".$cache_path);
		...
		function delete_group($group = NULL) {
			...
			foreach($map AS $file) {
				if(strpos($file, $group) !== FALSE) unlink($this->_path.$file);
			}
